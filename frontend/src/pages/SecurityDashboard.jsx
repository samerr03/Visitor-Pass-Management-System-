...
const SecurityDashboard = () => {
    ...
    const handlePrint = useReactToPrint({
        ...
        documentTitle: `Visitor-Pass-${currentPass?.passId}`,
        onBeforeGetContent: () => {
            // console.log("Preparing to print...", componentRef.current);
    ...
    const processCheckout = async (id) => {
        ...
        try {
            ...
            // Show custom success toast logic here if needed, keeping it simple for now in existing msg area?
            // Or create a new toast state. For now, let's reuse 'msg' but maybe it needs to be global or separate.
            // Since msg is stuck in the form area, let's just log or maybe add a small toast system later.
            // For now, let's just rely on the UI update (row disappearing).
        } catch (err) {
                ...
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4 animate-fade-in no-print">
                    <div className="bg-white rounded-xl shadow-2xl max-w-md w-full flex flex-col max-h-[90vh] overflow-hidden transform transition-all scale-100">
                        ...
                        <div className="p-6 bg-slate-200 overflow-y-auto flex-1 flex items-center justify-center">
                            {/* Printable Component */}
                            <VisitorPass ref={componentRef} visitor={currentPass} />
                        </div>
